...
def bbb_conv3x3(in_planes, out_planes, stride=1):
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = bbb_conv3x3(inplanes, planes, stride)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = bbb_conv3x3(planes, planes)
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = bbb_conv3x3(width, width, stride, groups, dilation)
...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_features=False):
        ...
        x = self.fc(features)  # if model.fc = nn.Identity() done during training, the output will be [128, 64] because fc is just an identity function
        if return_features:
            return x, features
        return x
...
def bbbresnet18(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
